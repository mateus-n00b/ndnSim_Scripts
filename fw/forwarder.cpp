...
#include "ns3/node.h"
#include "copy-packet.h"
#include <boost/random/uniform_int_distribution.hpp>

NS_LOG_COMPONENT_DEFINE("ndn.fw.forwarder");
...
Forwarder::onIncomingInterest(Face& inFace, const Interest& interest)
{
	...
	auto clone = cloneInterest(interest);
	// clone->setNextForwarder(0); // My new field that holds the last Forwarder ID
  // clone->setLastForwarder(0);

	NS_LOG_INFO(ns3::Simulator::GetContext() << ".incoming interest " << interest.getName());

	interest.setTag(make_shared<lp::IncomingFaceIdTag>(inFace.getId()));
	clone->setTag(make_shared<lp::IncomingFaceIdTag>(inFace.getId()));
...
Forwarder::onOutgoingInterest(const shared_ptr<pit::Entry>& pitEntry, Face& outFace, const Interest& interest)
{
	...
	NS_LOG_INFO(ns3::Simulator::GetContext()<<
			"onOutgoingInterest face=" << outFace.getId() << " interest=" << pitEntry->getName());

  // NS_LOG_UNCOND("node("<<ns3::Simulator::GetContext() <<
  //     ").onOutgoingInterest face=" << outFace.getId() << " interest=" << interest.getName()
...
Forwarder::onInterestUnsatisfied(const shared_ptr<pit::Entry>& pitEntry)
{
	...
	NS_LOG_INFO(ns3::Simulator::GetContext()<<".onInterestUnsatisfied interest=" << pitEntry->getName());
...
///////////////////////////////////////////////////
//	Create a copy of data packet insted of look for incoming data
///////////////////////////////////////////////////
void
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
	...
	NS_LOG_INFO(ns3::Simulator::GetContext()<< ".onIncomingData data=" << data.getName());
...
Forwarder::onOutgoingData(const Data& data, Face& outFace)
{
	...
	NS_LOG_INFO(ns3::Simulator::GetContext()<<".onOutgoingData face=" << outFace.getId() << " data=" << data.getName());
